...
abstract class ReaderIteratorExBase[OUT] extends Iterator[OUT]{
  def read(buffer: Array[OUT], dataType: DataType): Int
  def getStream: DataInputStream
  def handleRead(readType: Int): Array[Byte]
}
/**
...
abstract class BasePythonRunner[IN, OUT](
  ...
  def compute(
               ...
               partitionIndex: Int,
               context: TaskContext): ReaderIteratorExBase[OUT] = {
  ...
  abstract class ReaderIterator(
                                 ...
                                 releasedOrClosed: AtomicBoolean,
                                 context: TaskContext)
    extends ReaderIteratorExBase[OUT] {

    private var nextObj: OUT = _
    private var eos = false
    def read(buffer: Array[Byte], dataType: DataType): Int = {
      1
    }
    override def getStream: DataInputStream = stream
    ...
    protected def handleEndOfDataSection(): Unit = {
      ...
      val eosInt = stream.readInt()
      if (eosInt == SpecialLengths.END_OF_STREAM) {
        if (reuseWorker && releasedOrClosed.compareAndSet(false, true)) {
...
private[spark] object PythonRunner {
  ...
  def apply(func: PythonFunction, dag: String): PythonRunner = {
    new PythonRunner(Seq(ChainedPythonFunctions(Seq(func))), dag)
  }
}
...
class PythonRunner(funcs: Seq[ChainedPythonFunctions],
  ...
  protected override def newWriterThread(
    ...
    new WriterThread(env, worker, inputIterator, partitionIndex, context) {
      ...
      protected override def writeIteratorToStream(dataOut: DataOutputStream): Unit = {
        ...
        dataOut.writeInt(dag.length)
        dataOut.write(dag.getBytes)
        // The server expects to get END_OF_DATA_SECTION only if we
        // do not want to reuse the worker instance.
        // In our case we will deviate and skip sending the end of data here.
        // And let the END_OF_STREAM be sent above in run()
        // dataOut.writeInt(SpecialLengths.END_OF_DATA_SECTION)
      }
    }
  }
  class ReaderIteratorEx(stream: DataInputStream,
                         writerThread: WriterThread,
                         startTime: Long,
                         env: SparkEnv,
                         worker: Socket,
                         ...
                         releasedOrClosed: AtomicBoolean,
                         context: TaskContext)
    extends ReaderIterator(stream, writerThread, startTime,
      env, worker, pid, releasedOrClosed, context) {

    private val logger = LoggerFactory.getLogger(getClass)
    override def read(buffer: Array[Array[Byte]], dataType: DataType): Int = {
      1
    }
    override def handleRead(readType: Int): Array[Byte] = {
      try {
        readType match {
          ...
          case SpecialLengths.TIMING_DATA =>
            logger.info("handleRead: Reading timing data")
            handleTimingData()
            read()
          case SpecialLengths.PYTHON_EXCEPTION_THROWN =>
            throw handlePythonException()
          case SpecialLengths.END_OF_DATA_SECTION =>
            logger.info("handleRead: End of data")
    ...
    protected override def read(): Array[Byte] = {
      ...
      try {
        stream.readInt() match {
          ...
          case 0 => Array.emptyByteArray
          case SpecialLengths.TIMING_DATA =>
            logger.info("read: found timing data")
            handleTimingData()
            read()
          case SpecialLengths.PYTHON_EXCEPTION_THROWN =>
            logger.info("read: found exception")
            throw handlePythonException()
          case SpecialLengths.END_OF_DATA_SECTION =>
            logger.info("read: found end of data")
            handleEndOfDataSection()
            null
        }
      } catch handleException
  ...
  protected override def newReaderIterator( stream: DataInputStream,
                                            writerThread: WriterThread,
                                            startTime: Long,
                                            env: SparkEnv,
                                            worker: Socket,
                                            ...
                                            releasedOrClosed: AtomicBoolean,
                                            context: TaskContext):
    ReaderIteratorExBase[Array[Byte]] = {
    new ReaderIteratorEx(stream, writerThread, startTime, env, worker,
                         pid, releasedOrClosed, context)
  }
}
