...
object GenericPushdownOptimizationRule extends Rule[LogicalPlan] {
  ...
  def canHandlePlan(project: Seq[NamedExpression],
    ...
    if (attrReferencesEither.isLeft) {
      ...
      if (filterReferencesEither.isLeft) {
        ...
        logger.warn("Plan has no filters for " + (scanArgs._3).get("currenttest"))
        false
      } else {
        true
      }
  ...
  : LogicalPlan = {
    ...
    val ndpString = getNdpPythonString()
    logger.info(s"Get NDP String ${ndpString}")
    opt.put("ndpPython", ndpString)
  ...
  private def transformProjFiltAggregate(projectTop: Seq[NamedExpression],
    ...
    val aggOpts = aggScan match {
      ...
      case GenericPushdownScan(schema, opts) => opts
    }
